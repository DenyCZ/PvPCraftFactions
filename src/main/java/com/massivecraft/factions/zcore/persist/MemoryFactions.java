...
public abstract class MemoryFactions extends Factions {
    ...
    public void load() {
        ...
        if (!factions.containsKey("-3")) {
            Faction faction = generateFactionObject("-3");
            factions.put("-3", faction);
            faction.setTag(TL.KOTH.toString());
            faction.setDescription(TL.KOTH_DESCRIPTION.toString());
        } else {
            Faction faction = factions.get("-3");
            if (!faction.getTag().equalsIgnoreCase(TL.KOTH.toString())) {
                faction.setTag(TL.KOTH.toString());
            }
            if (!faction.getDescription().equalsIgnoreCase(TL.KOTH_DESCRIPTION.toString())) {
                faction.setDescription(TL.KOTH_DESCRIPTION.toString());
            }
            // if KoTH has old pre-1.6.0 name, rename it to remove troublesome " "
            //Should never happen
            if (faction.getTag().contains(" ")) {
                faction.setTag(TL.KOTH.toString());
            }
    ...
    @Override
    public Faction getKoTH() {
    	return factions.get("-3");
    }
